...
{
//#ifdef _DEBUG
	if (g_pShaderAPIDx11->GetBoundMaterial())
		return g_pShaderAPIDx11->GetBoundMaterial()->PerformDebugTrace();
//#endif
...
void CBaseMeshDX11::Spew( int nVertexCount, int nIndexCount, const MeshDesc_t &spewDesc )
{
	...
	for ( i = 0; i < nVertexCount; ++i )
	{
		...
		if ( numBoneWeights > 0 )
		{
			...
			{
				float weight = (*(float*)&(&pWeight)[j]); // evil casting
				temp += sprintf( temp, "%1.2f ", weight);
			}
		}
...
void CMeshMgr::GetMaxToRender( IMesh *pMesh, bool bMaxUntilFlush, int *pMaxVerts, int *pMaxIndices )
{
	...
	if ( !bMaxUntilFlush )
	{
		*pMaxVerts = ShaderAPI()->GetCurrentDynamicVBSize() / pVertexBuffer->VertexSize();
		if ( *pMaxVerts > VERTEX_BUFFER_SIZE)
		{
			*pMaxVerts = VERTEX_BUFFER_SIZE;
		}
...
int CMeshMgr::GetMaxVerticesToRender( IMaterial *pMaterial )
{
	...
	if ( nMaxVerts > VERTEX_BUFFER_SIZE)
	{
