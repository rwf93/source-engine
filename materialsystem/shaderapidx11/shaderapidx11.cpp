...
void CShaderAPIDx11::TexLodClamp( int finest ) {
	LOCK_SHADERAPI();
}

void CShaderAPIDx11::TexLodBias( float bias ) {
	LOCK_SHADERAPI();
}

void CShaderAPIDx11::CopyTextureToTexture( ShaderAPITextureHandle_t srcTex, ShaderAPITextureHandle_t dstTex ) {
	CTextureDx11 *pSrcTex = &GetTexture( srcTex );
	CTextureDx11 *pDstTex = &GetTexture( dstTex );
	// TODO(rwf93): Check mipmap levels and format.
				 ...
				 ImageFormat srcFormat, bool bSrcIsTiled, void *imageData )
{
	...
	info.m_pTexture = (ID3D11Resource*)GetD3DTexture( hModifyTexture );
...
IDirect3DBaseTexture *CShaderAPIDx11::GetD3DTexture( ShaderAPITextureHandle_t handle )
{
	...
	CTextureDx11 &tex = GetTexture( handle );
	if ( tex.m_NumCopies == 1 )
		return (IDirect3DBaseTexture*)tex.GetTexture();
	else
		return (IDirect3DBaseTexture*)tex.GetTexture( tex.m_CurrentCopy );
}
