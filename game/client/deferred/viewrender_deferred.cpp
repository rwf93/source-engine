...
void CDeferredViewRender::ViewDrawSceneDeferred( const CViewSetup &view, int nClearFlags, view_id_t viewID, bool bDrawViewModel )
{
	...
	if ( r_deferred_radiosity_debug.GetBool() )
		DebugRadiosity( view );
#endif
...
void CDeferredViewRender::PerformLighting( const CViewSetup &view )
{
	...
	const bool bRadiosityEnabled = DEFCFG_ENABLE_RADIOSITY != 0 && r_deferred_radiosity_enable.GetBool();
	...
	if ( GetGlobalLight() != NULL )
	{
		...
		if ( !GetLightingEditor()->IsEditorLightingActive() &&
			r_deferred_override_global_light_enable.GetBool() )
		{
			lightDataState.bShadow = r_deferred_override_global_light_shadow_enable.GetBool();
			UTIL_StringToVector( lightDataState.diff.AsVector3D().Base(), r_deferred_override_global_light_diffuse.GetString() );
			UTIL_StringToVector( lightDataState.ambh.AsVector3D().Base(), r_deferred_override_global_light_ambient_high.GetString() );
			UTIL_StringToVector( lightDataState.ambl.AsVector3D().Base(), r_deferred_override_global_light_ambient_low.GetString() );
...
static int GetSourceRadBufferIndex( const int index )
{
	...
	const int iNumSteps = (bFar ? r_deferred_radiosity_propagate_count_far.GetInt() : r_deferred_radiosity_propagate_count.GetInt())
		+ (bFar ? r_deferred_radiosity_blur_count_far.GetInt() : r_deferred_radiosity_blur_count.GetInt());
...
void CDeferredViewRender::EndRadiosity( const CViewSetup &view )
{
	const int iNumPropagateSteps[2] = { r_deferred_radiosity_propagate_count.GetInt(),
		r_deferred_radiosity_propagate_count_far.GetInt() };
	const int iNumBlurSteps[2] = { r_deferred_radiosity_blur_count.GetInt(),
		r_deferred_radiosity_blur_count_far.GetInt() };
