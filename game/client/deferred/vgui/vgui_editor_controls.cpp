...
ConVar r_deferred_lighteditor_defaultvmfpath( "r_deferred_lighteditor_defaultvmfpath", "", FCVAR_ARCHIVE );
...
void CVGUILightEditor_Controls::OnLevelSpawn()
{
	...
	if ( pCheckDebugStats != NULL )
		pCheckDebugStats->SetSelected( r_deferred_lightmanager_debug.GetBool() );
}
...
void CVGUILightEditor_Controls::OnCheckButtonChecked( Panel *panel )
{
	...
	{
		r_deferred_lightmanager_debug.SetValue( ( bChecked ? 1 : 0 ) );
	}
}
...
void CVGUILightEditor_Controls::BuildVmfPath( char *pszOut, int maxlen, bool bMakeRelative )
{
	...
	if( Q_strcmp( r_deferred_lighteditor_defaultvmfpath.GetString(), "" ) != 0 &&
		g_pFullFileSystem->IsDirectory( r_deferred_lighteditor_defaultvmfpath.GetString() ) )
	{
		Q_strcpy( tmp, r_deferred_lighteditor_defaultvmfpath.GetString() );
	}
	else
	{
