...
KeyValues *def_light_t::AllocateAsKeyValues()
{
	...
	for ( int iField = 0; iField < iNumFields; iField++ )
	{
		...
		{
			Vector col;
			Q_memcpy( col.Base(), reinterpret_cast<byte*>( this ) + pField.fieldOffset[0], pField.fieldSizeInBytes );
		...
		else
		{
			...
			case FIELD_FLOAT:
				{
					float fl1 = 0;
					Q_memcpy( &fl1, reinterpret_cast<byte*>( this ) + pField.fieldOffset[0], pField.fieldSizeInBytes );
			...
			case FIELD_CHARACTER:
				{
					int it1 = 0;
					Q_memcpy( &it1, reinterpret_cast<byte*>( this ) + pField.fieldOffset[0], pField.fieldSizeInBytes );
		...
		for ( int iField = 0; iField < iNumFields; iField++ )
		{
			...
			if ( !Q_stricmp( pField.externalName, pszKeyName ) )
			{
					...
					!Q_stricmp( pszKeyName, GetLightParamName( LPARAM_AMBIENT ) ) )
				{
					...
					Q_memcpy( reinterpret_cast<byte*>( this ) + pField.fieldOffset[0], col.Base(), pField.fieldSizeInBytes );
				}
				else if ( !Q_stricmp( pszKeyName, GetLightParamName( LPARAM_COOKIETEX ) ) )
				{
					...
					if ( pCookieValidate != NULL )
					{
						...
						Q_memcpy( reinterpret_cast<byte*>( this ) + pField.fieldOffset[0], &iAddedCookie, pField.fieldSizeInBytes );
					}
				}
					...
					!Q_stricmp( pszKeyName, GetLightParamName( LPARAM_ANGLES ) ) )
				{
					...
					Q_memcpy( reinterpret_cast<byte*>( this ) + pField.fieldOffset[0], f, pField.fieldSizeInBytes );
				}
				else
				{
					switch ( pField.fieldType )
					...
					case FIELD_FLOAT:
						{
							...
							Q_memcpy( reinterpret_cast<byte*>( this ) + pField.fieldOffset[0], &fl1, pField.fieldSizeInBytes );
						}
						break;
					case FIELD_SHORT:
					case FIELD_CHARACTER:
						{
							...
							Q_memcpy( reinterpret_cast<byte*>( this ) + pField.fieldOffset[0], &it1, pField.fieldSizeInBytes );
						}
						break;
					}
				}
