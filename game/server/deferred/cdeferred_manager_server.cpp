...
float ComputeLightRadius( const dworldlight_version0_t &light )
{
...
static ConVar r_deferred_autoenvlight_ambient_intensity_low("r_deferred_autoenvlight_ambient_intensity_low", "0.15");
static ConVar r_deferred_autoenvlight_ambient_intensity_high("r_deferred_autoenvlight_ambient_intensity_high", "0.45");
static ConVar r_deferred_autoenvlight_diffuse_intensity("r_deferred_autoenvlight_diffuse_intensity", "1");
...
void CDeferredManagerServer::LevelInitPreEntity()
{
	...
	if ( lightLump.uncompressedSize )
	{
		...
		{
			if ( lightLump.uncompressedSize % sizeof( dworldlight_version0_t ) )
				return;

			lightCount = lightLump.uncompressedSize / sizeof( dworldlight_version0_t );
			lights = new dworldlight_version0_t[lightCount];
	...
	{
		if ( lightLump.filelen % sizeof( dworldlight_version0_t ) )
			return;

		g_pFullFileSystem->Seek( hFile, lightLump.fileofs, FILESYSTEM_SEEK_HEAD );

		lightCount = lightLump.filelen / sizeof( dworldlight_version0_t );
...
#define COPY_LIGHT_DATA( name ) pair.spotlight->SetString( name, pair.light->GetString( name ) )
	...
	CUtlVector<const dworldlight_version0_t*> unspawnedLights;
	FOR_EACH_TRUE_SUBKEY( vmfFile, entity )
	{
		...
		{
			const dworldlight_version0_t& light = lights[i];
	...
	for ( int i = 0; i < numUnspawnedLights; ++i )
	{
		const dworldlight_version0_t* light = unspawnedLights[i];
